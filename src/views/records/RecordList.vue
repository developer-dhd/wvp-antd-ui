...
export default {
        ...
        return getOnlineMediaServerList(requestParameters)
            .then(res => {
              ...
              if (this.mediaServerList.length > 0) {
                ...
                let port = this.mediaServerList[0].httpPort
                if (location.protocol === 'https:' && this.mediaServerList[0].httpSSlPort) {
                  port = this.mediaServerList[0].httpSSlPort
                }
                this.mediaServerPath = location.protocol + '//' + this.mediaServerList[0].streamIp + ':' + port
              }
            })
            .then(() => {
            ...
            .then(data => {
              requestParameters.list = data
              requestParameters.NVR = 'deviceChannel'
              //console.log(resetRecordList(requestParameters))
  ...
  methods: {
    goDetailPage(record) {
      ...
      record.mediaServerPath = this.mediaServerPath
    ...
    handleChange(val) {
      this.mediaServerList.forEach(item => {
        if (item.id === val) {
          this.mediaServerPath = location.protocol + '//' + item.streamIp + ':' + item.httpPort
        }
      })
