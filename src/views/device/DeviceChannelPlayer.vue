<template>
  <a-modal
      ...
      :footer="null"
      @cancel="close()">
    <live-player :video-url="videoUrl" :hasaudio="hasAudio" :alt="playerAlt" autoplay live fluent stretch/>
...
export default {
  ...
  methods: {
    ...
    openDialog(channelInfo) {
      this.channelId = channelInfo.channelId
      this.deviceId = channelInfo.deviceId
      this.hasAudio = channelInfo.hasAudio
      this.streamId = ''
      this.mediaServerId = ''
      this.app = ''
      this.videoUrl = ''
      this.isShowPlayer = true
      this.noticeDevicePushStream() //通知设备推流
    },

    noticeDevicePushStream() {
      noticePushStream({ deviceId: this.deviceId, channelId: this.channelId }).then(res => {
        if (res.code === 0) {
    ...
    coverPlay() {
      ...
      coverPlay({ streamId: this.streamId }).then(res => {
        ...
        if (res.code === 0) {
          ...
          if (res.data) {
            setTimeout(() => {
              this.videoUrl = this.getUrlByStreamInfo(res.data)
              console.log('转码后视频地址：' + this.videoUrl)
            }, 2000)
          } else {
    ...
    convertStop(callback) {
      convertStop({ convertKey: this.convertKey, mediaServerId: this.mediaServerId }).then(res => {
        if (res.code === 0) {
          console.log('停止转码：' + res.msg)
        } else {
          console.log('停止转码：' + res.msg)
        }
